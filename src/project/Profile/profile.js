...
import {getSessionUser, getUser, blockUser, editUser} from "../../services/userService";
import {
    findAllReviews,
    findReviewsForUser,
    deleteReview,
...
export default class Profile extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            loggedInUser: {},
            editingMode: false,
            tempUser: {username: "", role: "", blocked: false, password: "", name: ""}
        };
    }
    ...
    setUsername = (evt) => {
        this.setState({tempUser: {...this.state.tempUser, username: evt.target.value}});
    };

    setPassword = (evt) => {
        this.setState({tempUser: {...this.state.tempUser, password: evt.target.value}});
    };

    setName = (evt) => {
        this.setState({tempUser: {...this.state.tempUser, name: evt.target.value}});
    };
        ...
        return (
            <div className="d-flex justify-content-center fill text-white">
                <div className="col-8">
                    ...
                    <button className="btn btn-primary" onClick={() => {this.setEditingMode(); this.setState({tempUser: this.state.user})}}>Edit profile</button>
                    }
                    {this.state.editingMode &&
                    <button className="btn btn-success" onClick={() => {this.setNormalMode(); editUser(this.state.tempUser).then(user => this.setState({user: user}))}}>Save edits</button>
                    }
                    {this.state.editingMode &&
